...
DEFAULT_CONFIG = {
    ...
    "Teams": {
        "webhook_url": "请替换为你的Teams Incoming Webhook URL"
    }
}
...
def process_evp_compliance_requests(evp_df, current_date):
    """处理EVP Compliance相关请求"""
    report_type = "EVP Compliance 情况和任务提醒"
    
    if evp_df.empty:
        print("信息：没有EVP Compliance相关的请求数据")
        empty_df = pd.DataFrame(columns=[
            'Action Owner', 'Action Owner Email', 'System Name', 'Category',
            '状态', '请求数量'
        ])
    ...
    # 获取负责人和邮箱
    def get_evp_owner_and_email(group):
        try:
            if 'Log Date' in group.columns and not group['Log Date'].isna().all():
                latest_log = group.loc[group['Log Date'].idxmax()]
                email = ensure_pg_email(
    ...
    try:
        grouped = evp_df.groupby('request_group')
        owner_info = grouped.apply(
            lambda x: pd.Series({
                'Action Owner': get_evp_owner_and_email(x)['Action Owner'],
                'Action Owner Email': get_evp_owner_and_email(x)['Action Owner Email'],
                'System/Solution': x['System/Solution'].iloc[0] if pd.notna(x['System/Solution'].iloc[0]) else '未知系统',
                'Category': x['Category'].iloc[0] if pd.notna(x['Category'].iloc[0]) else '未知分类',
                'Status': x['Status'].iloc[0] if pd.notna(x['Status'].iloc[0]) else '未知状态'
            })
        ).reset_index(drop=True)
        
        for col in ['Action Owner', 'Action Owner Email', 'System/Solution', 'Category', 'Status']:
            if col not in owner_info.columns:
                owner_info[col] = '未知'
                
    except Exception as e:
        print(f"EVP分组处理时出错: {str(e)}")
        owner_info = pd.DataFrame({
            'Action Owner': ['处理错误'],
            'Action Owner Email': [''],
            'System/Solution': ['未知系统'],
            'Category': ['EVP Compliance'],
            'Status': '未知状态'
        })

    target_table = owner_info.groupby(
        ['Action Owner', 'Action Owner Email', 'System/Solution', 'Category', 'Status']
    ).size().reset_index(name='请求数量')
    ...
    total_count = target_table['请求数量'].sum()
    total_row = pd.DataFrame([{
        'Action Owner': '总计',
        'Action Owner Email': '',
        'System Name': '',
    ...
    valid_recipients = list(set([ensure_pg_email(email) for email in config_recipients + data_recipients if email.strip()]))
    valid_cc = list(set([ensure_pg_email(email) for email in config_cc if email.strip()]))
    valid_recipients = [email for email in valid_recipients if email]
    valid_cc = [email for email in valid_cc if email]
...
def send_report(report_data, reminder_dir, log_dir):
    ...
    try:
        ...
        success = send_email(subject, email_html, to_addrs=recipients, cc_addrs=cc)
        if success:
            success_msg = f"{report_type}类型邮件发送成功"
            print(success_msg)
            log_message(success_msg, log_dir)
        else:
            fail_msg = f"{report_type}类型邮件发送失败"
            print(fail_msg)
            log_message(fail_msg, log_dir)
    except Exception as e:
...
def main():
    ...
    try:
        ...
        analyze_msg = "正在分析请求数据（包括Pending review、Revoked和EVP Compliance）..."
        print(analyze_msg)
